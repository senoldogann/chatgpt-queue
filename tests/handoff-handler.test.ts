...
describe('handleHandoffRequest', () => {
  it('opens a same-origin new chat and remembers it as the only import target', async () => {
    ...
    const result = await handleHandoffRequest(
      { type: 'handoffOpen', url: 'https://chatgpt.com/', handoffId: 'handoff:a' },
      7,
    ...
    expect(currentTarget()).toEqual({ tabId: 42, handoffId: 'handoff:a', createdAt: 1_000 });
  });

  it('refuses to open a cross-origin or non-http target', async () => {
    const { dependencies, created } = setup();

    await expect(handleHandoffRequest({ type: 'handoffOpen', url: 'https://evil.example/', handoffId: 'handoff:a' }, 7, 'https://chatgpt.com/c/abc', dependencies))
      .rejects.toThrow('handoff-url-not-allowed');
    await expect(handleHandoffRequest({ type: 'handoffOpen', url: 'javascript:alert(1)', handoffId: 'handoff:a' }, 7, 'https://chatgpt.com/c/abc', dependencies))
      .rejects.toThrow('handoff-url-not-allowed');
  ...
  it('only lets the tab the extension opened claim the import, once', async () => {
    ...
    await handleHandoffRequest({ type: 'handoffOpen', url: 'https://chatgpt.com/', handoffId: 'handoff:a' }, 7, 'https://chatgpt.com/c/abc', dependencies);

    expect(await handleHandoffRequest({ type: 'handoffClaim' }, 999, undefined, dependencies)).toEqual({ claimed: false });
    expect(await handleHandoffRequest({ type: 'handoffClaim' }, 42, undefined, dependencies)).toEqual({ claimed: true, handoffId: 'handoff:a' });
  ...
  it('expires a stale claim instead of leaking it to a later tab', async () => {
    const { dependencies, currentTarget } = setup({
      target: { tabId: 42, handoffId: 'handoff:a', createdAt: 0 },
  ...
  it('fails closed when the sender URL is unavailable', async () => {
    const { dependencies } = setup();
    await expect(handleHandoffRequest({ type: 'handoffOpen', url: 'https://chatgpt.com/', handoffId: 'handoff:a' }, 7, undefined, dependencies))
